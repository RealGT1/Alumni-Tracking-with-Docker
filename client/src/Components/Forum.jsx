...
const Forum = () => {
    ...
    useEffect(() => {
        const fetchMessages = async () => {
            try {
                const response = await axios.get('http://localhost:3000/auth/get_messages');
    ...
    const handleSubmit = async (event) => {
        ...
        try {
            const response = await axios.post('http://localhost:3000/auth/add_message', { content });
            if (response.data.Status) {
                setContent('');
            } else {
    ...
    return (
        <div className="flex flex-col w-full h-[94%] bg-gray-100 text-gray-800 p-9 overflow-x-hidden">
            <div className="flex flex-col flex-grow w-full max-w-xl bg-white shadow-xl rounded-lg ">
                ...
                <div className="bg-gray-300 p-3">
                    <form onSubmit={handleSubmit} className="flex">
                        <input
                            className="flex items-center h-11 w-full rounded px-3 text-base text-center"
                            value={content}
